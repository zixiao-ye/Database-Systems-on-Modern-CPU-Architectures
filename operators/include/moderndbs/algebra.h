...
class Select : public UnaryOperator {
  public:
    ...
    enum class AttrType { INT, STRING, INDEX };
  ...
  private:
    ...
    size_t attr_right_index;
    std::vector<Register*> regs;
    std::variant<int64_t, std::string> constant;
...
class Sort : public UnaryOperator {
  ...
  private:
    ...
    // offset for the current output tuple
    size_t offset;
...
class HashJoin : public BinaryOperator {
  private:
    ...
    std::vector<Register*> inputs_left;
    std::vector<Register*> inputs_right;
    std::vector<Register> outputs;

    std::unordered_map<Register, std::vector<Register>, RegisterHasher>
        map_from_attr_to_tuple;

  public:
    /// Constructor
    /// attr_index_left: the left key to compare as an index from
    /// input_left's output attr_index_right: the right key to compare as an
    /// index from input_right's output
    HashJoin(Operator& input_left, Operator& input_right,
...
class HashAggregation : public UnaryOperator {
  ...
  private:
    ...
    std::unordered_map<std::vector<Register>, std::vector<Register>,
                       RegisterVectorHasher>
        ht;

    decltype(ht)::iterator it;

    // true when the hash table build phase completed
    bool build = false;

  public:
    /// Constructor
    /// group_by_attrs: the input keys to build groups over as an index from
    /// input's output aggr_funcs: aggregations to compute, referencing
    /// attributes as an index from input's output
...
/// Computes the union of the two inputs with set semantics (removing
/// duplicates). This can be computed, using a hash set (e.g.,
/// std::unordered_set) that compares all attributes of the inpout.
...
class Intersect : public BinaryOperator {
  private:
    ...
    // check duplication for the right inputs
    std::unordered_set<std::vector<Register>, RegisterVectorHasher> ht_check;
...
class ExceptAll : public BinaryOperator {
  private:
    ...
    std::unordered_map<std::vector<Register>, uint64_t, RegisterVectorHasher>
        ht;
